...
const HeaderSection = ({t, router, section}:{t:any, router:any, section:'Tilda' | 'Pricing' | 'Solutions' }) => {
  ...
  return(
    <Box position='relative' onMouseEnter={() => setIsHovering(true)} onMouseLeave={() => setIsHovering(false)}> 
      <Flex alignItems={'center'} fontWeight={500} cursor={'pointer'}   gap='5px' color={isHovering?'gray.600':''}>
        <Text fontSize={{base: '.9em',sm:'.9em', md: '1em', lg: '1.1em'}} onClick={()=>{router.push(`/${section.toLowerCase()}`) }} >{section === 'Tilda'?section:t(section)}</Text>
      </Flex>    
    </Box>
)}
...
const Header=({isComputerWidth}:{isComputerWidth:boolean})=>{
  ...
  return( 
      <Flex width={'100vw'} justifyContent={'center'} position={'absolute'} top={0} left={0} zIndex={1000000}> 
          ...
          <Flex mt={'5vh'} color={blackTextSections.includes(pathname)?'brand.black_button':'white'}  bg={blackTextSections.includes(pathname)?'rgba(0, 20, 51, 0.1)':'rgba(256,256,256,0.3)'} borderRadius={'1rem'} px={{ base: '20px', sm: '30px', md: '50px' }}width={'100%'} maxW='1200px' height='8vh' justifyContent='space-between' alignItems='center'  >
            {isComputerWidth ? <>
              ...
              <Flex gap='4vw' alignItems={'center'}> 
                ...
                <Flex gap='2vw' > 
                  <Flex gap='16px' bg={blackTextSections.includes(pathname)?'brand.black_button':'white'}  color={blackTextSections.includes(pathname)?'white':'brand.black_button'}  py='5px' px='15px' borderRadius={'2rem'} cursor='pointer' alignItems={'center'}  _hover={{color:'gray.500'}}   onClick={() => window.open('https://superservice.matil.es/', '_blank')}> 
