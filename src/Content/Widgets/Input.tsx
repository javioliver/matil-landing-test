        ...
        <FormControl>
            <Box mt={mt as string} flexDir={{base:'column',sm:'column', md:'row'}} justifyContent={{base:'space-between',md:'space-between'}}  alignItems={{base:'normal',sm:'normal', md:'center'}}>
                ...
                <Box mt={'10px'} position="relative">
                    <Input    transition='border-bottom 0.3s ease-in-out'  fontWeight={300} fontSize={'md'}  id={`${placeholder}-input`} onChange={handleChange} value={value} placeholder={placeholder}  border={'none'} px={0} borderBottom={`3px solid ${isNotValid ? 'red':'rgb(211, 219, 229)'}`} size='sm'  bg='transparent' focusBorderColor='transparent'   _focus={{border:'none', borderBottom: `3px solid ${'rgb(5, 102, 255)'}`}}/>
                </Box> 
            </Box>
        </FormControl>
    );
